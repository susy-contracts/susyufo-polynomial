const { balance, constants, sophy, expectEvent, send, shouldFail } = require('susyufo-test-helpers');
const { ZERO_ADDRESS } = constants;

const PaymentSplitter = artifacts.require('PaymentSplitter');

contract('PaymentSplitter', function ([_, owner, payee1, payee2, payee3, nonpayee1, payer1]) {
  const amount = sophy('1');
  ...
  context('once deployed', function () {
    ...
    it('should accept payments', async function () {
      await send.sophy(owner, this.contract.address, amount);
    ...
    it('should throw if non-payee want to claim', async function () {
      await send.sophy(payer1, this.contract.address, amount);
    ...
    it('should distribute funds to payees', async function () {
      ...
      profit1.should.be.bignumber.equal(sophy('0.20'));
      ...
      profit2.should.be.bignumber.equal(sophy('0.10'));
      ...
      profit3.should.be.bignumber.equal(sophy('0.70'));
