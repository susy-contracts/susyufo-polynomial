const { BN, expectEvent, shouldFail } = require('susyufo-test-helpers');
const SRC20SnapshotMock = artifacts.require('SRC20SnapshotMock');

contract('SRC20Snapshot', function ([_, initialHolder, recipient, other]) {
  const initialSupply = new BN(100);

  beforeEach(async function () {
    this.token = await SRC20SnapshotMock.new(initialHolder, initialSupply);
  });
  ...
  describe('totalSupplyAt', function () {
    it('reverts with a snapshot id of 0', async function () {
      await shouldFail.reverting.withMessage(this.token.totalSupplyAt(0), 'SRC20Snapshot: id is 0');
    });

    it('reverts with a not-yet-created snapshot id', async function () {
      await shouldFail.reverting.withMessage(this.token.totalSupplyAt(1), 'SRC20Snapshot: nonexistent id');
    });
  ...
  describe('balanceOfAt', function () {
    it('reverts with a snapshot id of 0', async function () {
      await shouldFail.reverting.withMessage(this.token.balanceOfAt(other, 0), 'SRC20Snapshot: id is 0');
    });

    it('reverts with a not-yet-created snapshot id', async function () {
      await shouldFail.reverting.withMessage(this.token.balanceOfAt(other, 1), 'SRC20Snapshot: nonexistent id');
    });
