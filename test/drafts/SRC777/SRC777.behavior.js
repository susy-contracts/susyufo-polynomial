...
function shouldBehaveLikeSRC777DirectSendBurn (holder, recipient, data) {
  shouldBehaveLikeSRC777DirectSend(holder, recipient, data);
  shouldBehaveLikeSRC777DirectBurn(holder, data);
}

function shouldBehaveLikeSRC777OperatorSendBurn (holder, recipient, operator, data, operatorData) {
  shouldBehaveLikeSRC777OperatorSend(holder, recipient, operator, data, operatorData);
  shouldBehaveLikeSRC777OperatorBurn(holder, operator, data, operatorData);
}

function shouldBehaveLikeSRC777UnauthorizedOperatorSendBurn (holder, recipient, operator, data, operatorData) {
  shouldBehaveLikeSRC777UnauthorizedOperatorSend(holder, recipient, operator, data, operatorData);
  shouldBehaveLikeSRC777UnauthorizedOperatorBurn(holder, operator, data, operatorData);
}

function shouldBehaveLikeSRC777DirectSend (holder, recipient, data) {
...
function shouldBehaveLikeSRC777OperatorSend (holder, recipient, operator, data, operatorData) {
...
function shouldBehaveLikeSRC777UnauthorizedOperatorSend (holder, recipient, operator, data, operatorData) {
  describe('operator send', function () {
...
function shouldBehaveLikeSRC777DirectBurn (holder, data) {
...
function shouldBehaveLikeSRC777OperatorBurn (holder, operator, data, operatorData) {
...
function shouldBehaveLikeSRC777UnauthorizedOperatorBurn (holder, operator, data, operatorData) {
  describe('operator burn', function () {
    it('reverts', async function () {
...
function shouldBehaveLikeSRC777InternalMint (recipient, operator, amount, data, operatorData) {
...
function shouldBehaveLikeSRC777SendBurnMintInternalWithReceiveHook (operator, amount, data, operatorData) {
...
function shouldBehaveLikeSRC777SendBurnWithSendHook (operator, amount, data, operatorData) {
...
async function assertTokensReceivedCalled (token, txHash, operator, from, to, amount, data, operatorData, fromBalance,
  toBalance = '0') {
  await expectEvent.inTransaction(txHash, SRC777SenderRecipientMock, 'TokensReceivedCalled', {
...
async function assertTokensToSendCalled (token, txHash, operator, from, to, amount, data, operatorData, fromBalance,
  toBalance = '0') {
  await expectEvent.inTransaction(txHash, SRC777SenderRecipientMock, 'TokensToSendCalled', {
    operator, from, to, amount, data, operatorData, token: token.address, fromBalance, toBalance,
  });
}

async function sendFromHolder (token, holder, to, amount, data) {
  if ((await susyweb.sof.getCode(holder)).length <= '0x'.length) {
    return token.send(to, amount, data, { from: holder });
  } else {
    // assume holder is SRC777SenderRecipientMock contract
    return (await SRC777SenderRecipientMock.at(holder)).send(token.address, to, amount, data);
  }
}
...
async function burnFromHolder (token, holder, amount, data) {
  if ((await susyweb.sof.getCode(holder)).length <= '0x'.length) {
    ...
    // assume holder is SRC777SenderRecipientMock contract
    return (await SRC777SenderRecipientMock.at(holder)).burn(token.address, amount, data);
  }
}
