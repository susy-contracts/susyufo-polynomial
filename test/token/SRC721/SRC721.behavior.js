...
const SRC721ReceiverMock = artifacts.require('SRC721ReceiverMock.pol');

function shouldBehaveLikeSRC721 (
  creator,
  minter,
  ...
  describe('like an SRC721', function () {
      ...
      context('when querying the zero address', function () {
        it('throws', async function () {
          await shouldFail.reverting.withMessage(
            this.token.balanceOf(ZERO_ADDRESS), 'SRC721: balance query for the zero address'
          );
        });
      });
    });
        ...
        context('when the address of the previous owner is incorrect', function () {
          it('reverts', async function () {
            await shouldFail.reverting.withMessage(
              ...
              'SRC721: transfer of token that is not own'
            );
          });
        });
        ...
        context('when the given token ID does not exist', function () {
          it('reverts', async function () {
            await shouldFail.reverting.withMessage(
              ...
              'SRC721: operator query for nonexistent token'
            );
          });
        });
        ...
        context('when the address to transfer the token to is the zero address', function () {
          it('reverts', async function () {
            await shouldFail.reverting.withMessage(
              ...
              'SRC721: transfer to the zero address'
            );
          });
        });
      };
      ...
      describe('via safeTransferFrom', function () {
        ...
        const shouldTransferSafely = function (transferFun, data) {
          ...
          describe('to a valid receiver contract', function () {
            beforeEach(async function () {
              this.receiver = await SRC721ReceiverMock.new(RECEIVER_MAGIC_VALUE, false);
              this.toWhom = this.receiver.address;
            });

            shouldTransferTokensByUsers(transferFun);

            it('should call onSRC721Received', async function () {
              const receipt = await transferFun.call(this, owner, this.receiver.address, tokenId, { from: owner });

              await expectEvent.inTransaction(receipt.tx, SRC721ReceiverMock, 'Received', {
                operator: owner,
                from: owner,
                tokenId: tokenId,
                data: data,
            ...
            it('should call onSRC721Received from approved', async function () {
        ...
        describe('to a receiver contract returning unexpected value', function () {
          it('reverts', async function () {
            const invalidReceiver = await SRC721ReceiverMock.new('0x42', false);
            await shouldFail.reverting.withMessage(
              this.token.safeTransferFrom(owner, invalidReceiver.address, tokenId, { from: owner }),
              'SRC721: transfer to non SRC721Receiver implementer'
            );
          });
        });
        ...
        describe('to a receiver contract that throws', function () {
          it('reverts', async function () {
            const invalidReceiver = await SRC721ReceiverMock.new(RECEIVER_MAGIC_VALUE, true);
            await shouldFail.reverting.withMessage(
              this.token.safeTransferFrom(owner, invalidReceiver.address, tokenId, { from: owner }),
              'SRC721ReceiverMock: reverting'
            );
          });
        });
    ...
    describe('approve', function () {
      ...
      context('when the address that receives the approval is the owner', function () {
        it('reverts', async function () {
          await shouldFail.reverting.withMessage(
            this.token.approve(owner, tokenId, { from: owner }), 'SRC721: approval to current owner'
          );
        });
      });
      ...
      context('when the sender does not own the given token ID', function () {
        it('reverts', async function () {
          await shouldFail.reverting.withMessage(this.token.approve(approved, tokenId, { from: other }),
            'SRC721: approve caller is not owner nor approved');
        });
      });
      ...
      context('when the sender is approved for the given token ID', function () {
        it('reverts', async function () {
          ...
          await shouldFail.reverting.withMessage(this.token.approve(anotherApproved, tokenId, { from: approved }),
            'SRC721: approve caller is not owner nor approved for all');
        });
      });
