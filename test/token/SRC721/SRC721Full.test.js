const { BN, shouldFail } = require('susyufo-test-helpers');

const { shouldBehaveLikeSRC721 } = require('./SRC721.behavior');
const { shouldSupportInterfaces } = require('../../introspection/SupportsInterface.behavior');

const SRC721FullMock = artifacts.require('SRC721FullMock.pol');

contract('SRC721Full', function ([
  creator,
  ...accounts
]) {
  ...
  beforeEach(async function () {
    this.token = await SRC721FullMock.new(name, symbol, { from: creator });
  });

  describe('like a full SRC721', function () {
    beforeEach(async function () {
    ...
    describe('burn', function () {
      ...
      it('burns all tokens', async function () {
        ...
        await shouldFail.reverting.withMessage(
          this.token.tokenByIndex(0), 'SRC721Enumerable: global index out of bounds'
        );
      });
    });
    ...
    describe('metadata', function () {
      ...
      it('reverts when setting metadata for non existent token id', async function () {
        await shouldFail.reverting.withMessage(
          this.token.setTokenURI(nonExistentTokenId, sampleUri), 'SRC721Metadata: URI set of nonexistent token'
        );
      });
      ...
      it('reverts when querying metadata for non existent token id', async function () {
        await shouldFail.reverting.withMessage(
          this.token.tokenURI(nonExistentTokenId), 'SRC721Metadata: URI query for nonexistent token'
        );
      });
    });
    ...
    describe('tokenOfOwnerByIndex', function () {
      ...
      describe('when the index is greater than or equal to the total tokens owned by the given address', function () {
        it('reverts', async function () {
          await shouldFail.reverting.withMessage(
            this.token.tokenOfOwnerByIndex(owner, 2), 'SRC721Enumerable: owner index out of bounds'
          );
        });
      });
      ...
      describe('when the given address does not own any token', function () {
        it('reverts', async function () {
          await shouldFail.reverting.withMessage(
            this.token.tokenOfOwnerByIndex(another, 0), 'SRC721Enumerable: owner index out of bounds'
          );
        });
      });
      ...
      describe('after transferring all tokens to another user', function () {
        ...
        it('returns empty collection for original owner', async function () {
          ...
          await shouldFail.reverting.withMessage(
            this.token.tokenOfOwnerByIndex(owner, 0), 'SRC721Enumerable: owner index out of bounds'
          );
        });
      });
    });
    ...
    describe('tokenByIndex', function () {
      ...
      it('should revert if index is greater than supply', async function () {
        await shouldFail.reverting.withMessage(
          this.token.tokenByIndex(2), 'SRC721Enumerable: global index out of bounds'
        );
      });
