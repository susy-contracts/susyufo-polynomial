const { BN, balance, sophy, shouldFail, time } = require('susyufo-test-helpers');
...
contract('SampleCrowdsale', function ([_, deployer, owner, wallet, investor]) {
  ...
  it('should not accept payments before start', async function () {
    await shouldFail.reverting.withMessage(this.crowdsale.send(sophy('1')), 'TimedCrowdsale: not open');
    await shouldFail.reverting.withMessage(this.crowdsale.buyTokens(investor, { from: investor, value: sophy('1') }),
      'TimedCrowdsale: not open'
    );
  });

  it('should accept payments during the sale', async function () {
    const investmentAmount = sophy('1');
  ...
  it('should reject payments after end', async function () {
    ...
    await shouldFail.reverting.withMessage(this.crowdsale.send(sophy('1')), 'TimedCrowdsale: not open');
    await shouldFail.reverting.withMessage(this.crowdsale.buyTokens(investor, { value: sophy('1'), from: investor }),
      'TimedCrowdsale: not open'
    );
  });
  ...
  it('should allow refunds if the goal is not reached', async function () {
    ...
    await time.increaseTo(this.openingTime);
    await this.crowdsale.sendTransaction({ value: sophy('1'), from: investor, gasPrice: 0 });
    await time.increaseTo(this.afterClosingTime);
